...
- Tree entries follow the same pattern in `trees-data.js` (`id`, `commonName`, `scientificName`, `range`, `ecoregion`, `description`, `funFacts`, `image{src,credit,sourceUrl}`). `description` doubles as the publish switch: blank means the species stays out of rotation.
...
- Real: one trail entry (Mount LeConte via Alum Cave, hiked 2023-05-14, with Ahmad's own notes); four project entries with repo URLs.
- **Not yet supplied, and must not be invented:** trail photos (`images/trails/charlie_brown.jpg` is referenced but not in the repo yet), the body of any individual trail review page, a description for the Automated Network Compliance Manager repo (its GitHub page 404s — URL needs confirming), reading list entries, the real bio text, and every tree `description`/`funFacts`/`range`/`ecoregion`/photo (all 63 species are listed by name only).
